...
//see comment on top of section for api parameters
	...
	//query events by category
		...
		if (isset($query['overlap'])) {
			//we are getting the intersection
    ...
    // $events->passed_args = $query['order'];
    // $events->passedby_args = $query['orderby'];
    // $events->exec_args = $args;
	...
	foreach ( $events->posts as $ep) {
		...
		$ep->eventCategories = tribe_get_event_cat_slugs( $postId);
		$ep->eventStart = get_post_meta( $postId, '_EventStartDate');
		$ep->eventEnd = get_post_meta( $postId, '_EventEndDate');
		// $ep->eventStart = tribe_get_start_date( $postId );
		// $ep->eventEnd = tribe_get_start_date( $postId );
		//$ep->open = false;
		$ep->id = $postId;
...
//see comment on top of section for api parameters
    ...
    register_rest_route( 'tribe_events/v2', '/sss_events/(?P<start_date>[a-z0-9\-]+)/(?P<end_date>[a-z0-9\-]+)/cat/(?P<categoryid>[0-9]+)/', array(
        'methods' => 'GET',
        'callback' => 'tribe_api_query',
    ) );
